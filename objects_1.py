...
ball_1=pygame.transform.scale(img_1,(20,20))
ball_2=pygame.transform.scale(img_2,(20,20))
class Circle(pygame.sprite.Sprite):
	#ĐÂY LÀ CÁC BALL VÒNG TRON BÊN NGOÀI
	def __init__(self, i):
...
class Player():
	...
	def reset(self):
		#radius là khoaản cách của playẻ với trục khi quay
		self.radius = 30
		self.theta = 0
		self.rotate = True
		#tốc độ bay của bóng tới enemi
		self.speed = 7
		self.dr = self.speed
		self.alive = True
	...
	def update(self, rotate,val_ball):
		...
		if val_ball==1:
			...
			ball=ball_2
		self.rect = ball.get_rect(center=(self.x, self.y))
		self.mask = pygame.mask.from_surface(ball)

	def draw(self, win,val_ball):
		if val_ball==1:
			ball=ball_1
		elif val_ball==2:
			ball=ball_2
		win.blit(ball, self.rect)

class Dot():
	#dot này là dot màu ăn điểm
	def __init__(self):
		self.radius = 8
